...
def preprocess(image, res=1024, dtype=torch.float16):
    image = image.resize((res, res), resample=Image.LANCZOS)
    image = np.array(image).astype(np.float32) / 255.0
    image = image[None].transpose(0, 3, 1, 2)
    image = torch.from_numpy(image)[:, :3, :, :].to(dtype=dtype, device='cuda')
    return 2.0 * image - 1.0

def encoder(image, model, res=1024):
    generator = torch.Generator().manual_seed(8888)
    vae_dtype = next(model.vae.parameters()).dtype
    image = preprocess(image=image, res=res, dtype=vae_dtype)
    gpu_generator = torch.Generator(device=image.device)
    gpu_generator.manual_seed(generator.initial_seed())
    return model.vae.config.scaling_factor * model.vae.encode(image).latent_dist.sample(generator=gpu_generator)

@torch.no_grad()
def ddim_reverse_sample(image, prompt, model, num_inference_steps=20, guidance_scale=2.5, res=1024):
    batch_size = 1

    # 为兼容性适配 StableDiffusionXLPipeline 的 encode_prompt 返回 dict 格式
    prompt_outputs = model.encode_prompt(
        prompt=prompt,
        device=model.device,
    ...
    added_cond_kwargs = {
        "text_embeds": prompt_outputs["pooled_prompt_embeds"],
        "time_ids": prompt_outputs["time_ids"]
    }
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        def forward(hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None, **kwargs):
            ...
            def reshape(t):
                b, l, d = t.shape
                h = self.heads
                t = t.view(b, l, h, d // h).permute(0, 2, 1, 3).reshape(b * h, l, d // h)
                return t
            ...
            def restore(t):
                b, l, d = t.shape
                h = self.heads
                t = t.view(b // h, h, l, d).permute(0, 2, 1, 3).reshape(b // h, l, d * h)
                return t
    ...
    def register_recursive(net_, count, place):
        if net_.__class__.__name__ == "CrossAttention":
            net_.forward = ca_forward(net_, place)
            return count + 1
        elif hasattr(net_, "children"):
            for child in net_.children():
                count = register_recursive(child, count, place)
        return count

    cross_att_count = 0
    for name, module in model.unet.named_children():
        if "down" in name:
            cross_att_count += register_recursive(module, 0, "down")
        elif "up" in name:
            cross_att_count += register_recursive(module, 0, "up")
        elif "mid" in name:
            cross_att_count += register_recursive(module, 0, "mid")
    controller.num_att_layers = cross_att_count


def reset_attention_control(model):
    def reset_recursive(net_):
        if hasattr(net_, "children"):
            for child in net_.children():
                reset_recursive(child)
        if hasattr(net_, "forward") and hasattr(net_.forward, "__wrapped__"):
            net_.forward = net_.forward.__wrapped__
        elif hasattr(net_, "original_forward"):
            net_.forward = net_.original_forward  # fallback if you saved manually

    for _, module in model.unet.named_children():
